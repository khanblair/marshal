...
describe("card notes", () => {
  ...
  it("writes a default note from the title and the area the card works in", () => {
    expect(defaultNote(cardOf(41))).toBe(
      [
  ...
  it("names the package when the card has one", () => {
    expect(defaultNote(cardOf(209))).toContain("- Keep the change inside apps/android");
  });

  it("shows the default note until one is saved", () => {
    expect(noteText(cardOf(41))).toBe(defaultNote(cardOf(41)));
    saveNote(cardOf(41), "Mine");
    expect(noteText(cardOf(41))).toBe("Mine");
  ...
  it("stores the default note the first time the card is drawn, and keeps a saved one", () => {
    ensureNote(cardOf(41));
    expect(M.S.notes?.[41]).toBe(defaultNote(cardOf(41)));
    saveNote(cardOf(41), "Mine");
    ensureNote(cardOf(41));
    expect(M.S.notes?.[41]).toBe("Mine");
  });

  it("places the note in the vault under the card number and a slug of the title", () => {
    expect(notePath(cardOf(41))).toBe("vault/cards/41-fix-token-refresh-on-login.md");
  });
});
