...
const portOf = (card: Card): number =>
  (isMobileProject(card) ? MOBILE_PORT_BASE : WEB_PORT_BASE) + (card.n % PORT_SPREAD);
...
export const previewState = (id: CardKey): PreviewState => M.S.preview?.[id] ?? "stopped";

/** Starts the fake dev server (it is up after a short wait) or stops it. */
export function togglePreview(id: CardKey): void {
  M.S.preview ??= {};
