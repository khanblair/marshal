...
const labels = (id: number, mobile = false): string[] =>
...
describe("cardActions", () => {
  ...
  it("puts the pending command approval first", () => {
    expect(cardActions(cardOf(44), false)[0]).toMatchObject({ label: "Approve", kbd: "A" });
  });

  it("offers Resume session, disabled while the card wakes", () => {
    expect(labels(115)).toContain("Resume session");
    cardOf(115).waking = true;
    const waking = cardActions(cardOf(115), false)[0];
    expect(waking).toMatchObject({ label: "Waking", disabled: true });
  });

  it("offers Resume card on a paused card", () => {
    cardOf(41).paused = true;
    expect(labels(41)[0]).toBe("Resume card");
  });

  it("offers Merge on a ready card, and no Pin on a done card", () => {
    expect(labels(36)).toContain("Merge");
    expect(labels(33)).toEqual(["Fork"]);
  });

  it("shows Unpin for a pinned card", () => {
    expect(labels(207)).toContain("Unpin");
  });

  it("queues a merge", () => {
    cardActions(cardOf(36), false)
      .find((a) => a.label === "Merge")
      ?.run();
    expect(cardOf(36)).toMatchObject({ state: "merging", mergePct: 10 });
...
describe("moreItems", () => {
  ...
  it("lists a move to every other column, then the fixed items", () => {
    const items = moreItems(cardOf(41), () => undefined);
  ...
  it("closes the menu before it runs an item", () => {
    const close = vi.fn();
    const items = moreItems(cardOf(41), close);
