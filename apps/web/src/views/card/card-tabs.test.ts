...
describe("cardTabs", () => {
  ...
  it("lists the seven tabs in order", () => {
    expect(cardTabs(cardOf(41)).map((tab) => tab.label)).toEqual([
      "Chat",
      "Comments",
      "Activity",
      "Diff",
  ...
  it("counts comments, activity, diff files, and done over total for checklists and checks", () => {
    const tabs = Object.fromEntries(cardTabs(cardOf(41)).map((tab) => [tab.key, tab.count]));
    expect(tabs.comments).toBe(cardOf(41).comments.length);
  ...
  it("leaves out the count when there is nothing to count", () => {
    const tabs = Object.fromEntries(cardTabs(cardOf(45)).map((tab) => [tab.key, tab.count]));
  ...
  it("marks the checklist tab failed when an acceptance check failed", () => {
    expect(cardTabs(cardOf(119)).find((tab) => tab.key === "checks")?.failed).toBe(true);
    expect(cardTabs(cardOf(41)).find((tab) => tab.key === "checks")?.failed).toBe(false);
  });
});
