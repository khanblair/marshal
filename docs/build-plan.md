...
| 2.27 | (moved from 5.10 and 5.16 on 2026-09-26, N4) Manual session controls: Pause holds a Working card between turns, Sleep stops the process and keeps the session id, Wake resumes it, Pin keeps a card from sleeping, and Start resumes a stopped, asleep, or paused card. The rules and messages are the prototype's (a working card cannot sleep until it is paused, a needs-you card stays awake, a card without an awake session cannot sleep, only a Working card can be paused). Screen work: none new, the existing controls call the daemon | The rules match the prototype's messages, a woken card keeps its context, and the states survive a daemon restart |
...
| 5.10 | Automatic sleep: the idle timer, grouped reminders, and awake limits (manual sleep, wake, and pin moved to 2.27) | Idle cards sleep and wake with context, working cards never sleep |
...
| 5.16 | (prototype, N4; moved to 2.27 on 2026-09-26) Pause and resume: hold a Working card between turns. A working card cannot sleep until it is paused, a needs-you card stays awake, and a card without an awake session cannot sleep. Screen work: Pause becomes Resume, and a paused indicator shows on the card and the board | The rules match the prototype's messages, and pause and resume work through the API |
