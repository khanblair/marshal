...
- **Layout by viewport:** the design switches layouts from `M.S.vw`, not CSS breakpoints. Components that change on phones take a `phone` flag (`Dialog`, `ToastRegion`, `ConnectionLost`, `SignIn`) or a `sheet` flag (`Menu`).
...
| `NotConnected` | A service that is not set up. Built on `EmptyState`: a plug icon, "{service} is not connected.", the reason, and a Connect button. It never shows sample rows or numbers | `service: string`, `reason?: string`, `onConnect: () => void`, `connectLabel?: string` ("Connect {service}"), plus `EmptyState` attributes | Not in the design: the prototype has no daemon, so it never draws these. Needed once the app talks to a real one (backend inventory N29, build plan 1.16). See ui-rules 5.6 | Built |
| `ErrorState` | One section that failed to load: a warning icon, the caller's plain message (plain text, not an alert, so several failed sections do not each interrupt a screen reader), Try again when there is something to retry, and the technical text inside an expandable Details. Built on `EmptyState`. It is the `ErrorMessage` the old plan named | `message: string`, `details?: string`, `onRetry?: () => void`, plus `EmptyState` attributes | Not in the design: the prototype has no daemon, so it never draws these. Needed once the app talks to a real one (backend inventory N29, build plan 1.16). See ui-rules 5.6 | Built |
| `Skeleton` | One rounded pulsing block that stands in for content while it loads. It is `aria-hidden` and uses the pulse token, which the base CSS stops under reduced motion | `width?: number \| SkeletonPercent` (full width by default; a number is px, and a percent string such as `"60%"` is a share of the container), `height?: number` (12), `circle?: boolean`, `style?: JSX.CSSProperties`, plus span attributes | Not in the design: the prototype has no daemon, so it never draws these. Needed once the app talks to a real one (backend inventory N29, build plan 1.16). See ui-rules 5.6 | Built |
| `SkeletonLines` | A paragraph while it loads: bars with the last one shorter | `lines?: number` (3, at least 1), `lineHeight?: number` (12), plus div attributes | Same as `Skeleton` | Built |
| `SkeletonRow` | The shape of a list or project row while it loads: an avatar or a dot, a title bar, and a shorter meta bar | `leading?: "avatar" \| "dot"` ("avatar": 28 px; dot: 8 px), plus div attributes | Same as `Skeleton` | Built |
| `SkeletonCard` | The shape of a board card while it loads: the card's own box and 3 px left edge, a two line title, a state line, and an activity line | div attributes | Same as `Skeleton` | Built |
| `SkeletonGroup` | Wraps the skeleton shapes of one loading part in one `role="status"` with a visually hidden "Loading", so a screen reader hears one message and not one per block. The caller marks the container it loads `aria-busy="true"` | `label?: string` ("Loading"), plus div attributes; it adds no layout | Same as `Skeleton` | Built |
...
| `VirtualList` | A tall list that draws only the rows near the screen (ui-rules section 8: virtualize any list that can pass 100 items). It is its own scroller and puts `before` above the rows, inside it, so a summary line scrolls with them. Rows are absolutely placed by their measured heights (an estimate until drawn), so a row can be taller when it is open. The row that has focus stays drawn when scrolled away, and each row is a `li` with `aria-posinset` and `aria-setsize`. The windowing arithmetic is in `virtual-window.ts` | `items: readonly T[]`, `itemKey: (item: T) => string`, `estimateHeight: (item: T) => number`, `gap?: number` (0, px), `overscan?: number` (400, px), `label?: string`, `before?: JSX.Element`, `handle?: (handle: { scrollToIndex(index) }) => void`, children `(item: Accessor<T>, index: number) => JSX.Element`, plus div attributes; `class` lays out the scroller | Card Diff tab file list | Built |
...
| `ConnectionLost` | Full screen for an app that has no data yet and cannot reach the daemon: a server icon, "Can't reach the daemon", one sentence on what to check, Try again, and "Trying again in 4 s" outside the alert. It fills its container and works at 320 px. Details hold technical text only | `onRetry: () => void`, `retryInSeconds?: number \| null` (null or missing shows no countdown; 0 shows `0 s`), `details?: string`, `phone?: boolean`, plus div attributes | Not in the design: the prototype has no daemon, so it never draws these. Needed once the app talks to a real one (backend inventory N29, build plan 1.16). See ui-rules 5.6 | Built |
| `OfflineBanner` | Thin warning bar at the top of the app for when it already has data and the connection dropped. `role="status"`, polite; the countdown is `aria-live="off"`. Built on `Callout`. It does not promise queued changes | `retryInSeconds?: number \| null`, plus div attributes | Not in the design: the prototype has no daemon, so it never draws these. Needed once the app talks to a real one (backend inventory N29, build plan 1.16). See ui-rules 5.6 | Built |
| `SignIn` | Full screen form to paste the access token: password field (`autocomplete="off"`, no spell check, focused on open), a Sign in button that is disabled while the token is blank or `busy`, Enter submits, and the error under the field with `role="alert"` and `aria-describedby`. It trims the token, and keeps it only in the field | `onSubmit: (token: string) => void`, `busy?: boolean`, `error?: string`, `phone?: boolean`, plus div attributes | Not in the design: the prototype has no daemon, so it never draws these. Needed once the app talks to a real one (backend inventory N29, build plan 1.16). See ui-rules 5.6 | Built |
...
The design does not have these, so they are not built: `Tooltip` (the design uses `title`), `Popover` (menus and panels cover it), `Spinner` (now `Icon name="spinner"`), `Tabs` (now `SegmentedControl kind="tabs"`), `ResponsiveTable` and `DataTable` (views switch between a table and a list themselves), `MarkdownContent`, `CodeBlock`, `TerminalView` (no xterm; the design's terminal is text lines), `SmellFindings`, `SmellFindingItem`, `DismissReasonDialog`, `ResourcePanel`, `TemplateEditor`, `SmellProfileEditor`, `TestConnectionButton`, `ConnectionTestResult`, `McpServerRow`, `SkillRow`, `Sparkline`, `MentionSuggest`, `EvidenceLink`, and `AddChecklistMenu`.

`Skeleton` and `ErrorMessage` were on this list because the design draws no loading or error screens. They exist now, because the prototype had no daemon and an app that talks to one has loading, failed, offline, signed out, and not connected states (backend inventory N29, build plan 1.16). `Skeleton` is in `base/` with its parts `SkeletonLines`, `SkeletonRow`, `SkeletonCard`, and `SkeletonGroup`; `ErrorMessage` is `ErrorState`, which shows the message and Details instead of a bare line of text. The full screens and the banner are in the layout table (`ConnectionLost`, `SignIn`, `OfflineBanner`), and `NotConnected` is in the base table. When to use each is in ui-rules 5.6.
