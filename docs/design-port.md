...
| A card is known by its project and number, such as `api#41`, and the visible label stays `#41`. Lists that can mix projects (the notices panel, the command palette, "Agents awake" and "Coming up today" on Home, and the toast and screen reader texts about a card) show the project name next to the number, such as `api-gateway #41`. New cards are numbered per project, so a new api card is `#47`, not `#300`. | Decided by the owner (D6): each project numbers its own cards, so the number alone is not unique (`backend-inventory.md` N26, `backend-checklist.md` B1.15). Seeded cards keep their numbers, so one project reads exactly as the prototype does. The list of places is the one the checklist names (Home, notices, search, the palette, the Agents view, chats). |
| The New project dialog never guesses what the repository is: the line under the fields keeps its plain sentence ("Choose a folder or paste a URL. Marshal detects the language and monorepo tools.") instead of "Detected a Go project" made from the text typed. A refusal from the daemon (not a Git repository, already added, clone failed) shows as its own plain sentence in a warning line under the fields, and the dialog stays open with the fields as they were | The daemon reads the language and monorepo tools when it adds the project, so a guess before that would be made up. What it found shows on the project's board. Cutover of S3, 2026-09-25. |
| The New project dialog has no Choose folder button in the web build, and the folder is a typed path (a leading `~` works) | A web page cannot open a native folder picker. The desktop app adds the button. The prototype's button filled a made-up folder. |
| A project from the daemon shows no CI dot in the sidebar, and Home's CI health and its "all" page say "GitHub is not connected. CI runs appear here once GitHub is connected." with a Connect GitHub button, where the prototype showed made-up runs | The daemon has no CI data until Phase 6, and the rules forbid made-up numbers or a made-up "queued" state (`backend-checklist.md` 2.3, rule 4). A project that does have CI data draws exactly as before. |
| A card whose package the project does not list gets its own swimlane (after the listed ones, before "No package") and its own package filter option, where the prototype gave it no lane | The daemon lists the packages it finds on disk, which can differ from the ones a card names, and a card must never disappear from its board. |
| Three screens the prototype did not draw: a loading skeleton of the app while the first data arrives, "Can't reach the daemon" (full screen), and "Sign in" (full screen, with the plain rejection sentence under the field); plus a thin offline bar above the app while it reconnects | The app now talks to a daemon that can be down or can refuse a token (`ui-rules.md` 5.6). They use existing tokens and components. |
| The New project dialog shows a "Branch" field only when cloning, empty by default (placeholder "Repository default"), where the prototype showed "Default branch" (`main`) for both sources | The daemon reads a folder's own default branch and uses the field only for a clone, where a default of `main` would fail a repository with another default. Approved 2026-09-25. |
| Under 360 px wide, the two source options of the New project dialog drop their icons | The labels wrapped at 320 px (found by the end-to-end checks). Approved 2026-09-25. |
| Project settings shows exactly what the daemon has for the default branch and the dev command: an empty dev command stays empty, where the prototype filled `pnpm dev` | A default the person never chose must not be saved as if they had. Approved 2026-09-25. |
| An agent that is not installed stays in the agent pickers (New card, a card's settings, and the role editor) but is disabled and labelled "(not installed)", and the daemon's install sentence (such as "Codex is not installed. Install it with: npm install -g @openai/codex") shows as small text under the pickers, where the prototype's list had only agents that worked | The pickers now read the daemon's agent catalog, which reports each agent as supported, untested, or missing. A missing agent must be visible with how to get it, and it cannot be chosen (build plan 1.14). Cutover of S4, 2026-09-25. |
| Dragging a card to In review or to Ready to merge is refused, with the prototype's own sentence, unless the card already has a pull request and (for Ready to merge) passing checks, where the prototype let any started card with a branch move to In review and invented a pull request number | The daemon cannot open a real pull request or run checks until Phases 5 and 6, and a card must not claim a review that does not exist. The refusal, its message, and its stable reason are in `architecture.md` 6.1. Approved 2026-09-26. |
| Upload image opens the operating system's file chooser, and no Remove-avatar control is drawn (the prototype's button only showed a toast and drew no remove) | The prototype had no real file to choose and no remove affordance at all. The web app can hand the chosen file's bytes to the daemon (`POST /v1/me/avatar`, a raw-body upload whose kind is read from the bytes), so the button now does what it says and the chosen image draws. The remove path exists on the daemon and in the client (`removeAvatar`) but draws nothing, because the design draws no control for it. Proposed 2026-09-26; awaiting the owner's sign-off. |
| The avatar picture draws in the header (the avatar menu) and on the Profile screen, and the Profile time-zone select offers "Not set" and the person's own zone beside the design's six | The design's avatar is initials only, and its time-zone select lists six zones. A person who uploads a picture expects to see it, and the upload is real now, so `@marshal/ui`'s `Avatar` gained an optional `src` (an `<img>` filling the circle) and draws initials when it is absent. The daemon takes any IANA time-zone name, so the select adds "Not set" when there is none and the person's own zone when it is one the design's six do not list; the six are unchanged. Proposed 2026-09-26; awaiting the owner's sign-off. |
