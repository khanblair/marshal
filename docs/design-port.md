...
| The onboarding project step offers only "Pick a folder" and "Clone from GitHub", and starts on "Pick a folder", where the prototype also offered "Use a sample project" (its default) and said the sample is safe to try things on | The sample project needs a sample repository that the daemon can add, and none exists until build plan task 2.25 (Phase 2). The option comes back then. Approved 2026-09-25. |
