...
func TopicKindValues() []TopicKind {
	return []TopicKind{TopicKindHome, TopicKindProject, TopicKindCard, TopicKindChat}
}
...
// ParseTopic splits a topic into its kind and id. The id is empty for the home topic. It checks
// the shape of the id (a project id or an opaque id) so a subscription cannot hold junk.
func ParseTopic(topic Topic) (TopicKind, string, error) {
	...
	if !found || kind == TopicKindHome || !kind.Valid() {
